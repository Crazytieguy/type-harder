...
export const tokenizeMarkdown = (text: string): Token[] => {
  ...
  while (pos < text.length) {
    ...
    // Check for italic
    if (text.substring(pos).startsWith('*') && !text.substring(pos).startsWith('**')) {
...
const renderToken = (token: Token, key: string): JSX.Element => {
  switch (token.type) {
    ...
    case 'link':
      return (
        <a key={key} href={token.url} target="_blank" rel="noopener noreferrer" className="link link-primary">
